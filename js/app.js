...
const inventorsBornBetween1500To1599 = inventors.filter((inventor) => {
    return inventor.year > 1500 && inventor.year < 1600
})

veryOldInventors = inventorsBornBetween1500To1599 
// Check your work:
...
const fullNameObjs = inventors.map((inventor) => {
    return {first: inventor.first, last: inventor.last}
})

inventorNames = fullNameObjs
...
const findAda = inventors.find(inventor => inventor.first === 'Ada')

inventorNamedAda = findAda
// Check your work:
...
const switchNameOrder = people.map((p) => p.split(', ').reverse().join(', '))

firstLast = switchNameOrder
...
const sumAllYears = inventors.reduce((acc, inventor) => {
    acc += inventor.passed - inventor.year
    return acc
}, 0)

totalYearsLived = sumAllYears
...
const tally = travelMethods.reduce((acc, method) => {
    if (acc[method]) {
        acc[method]++
    } else {
        acc[method] = 1
